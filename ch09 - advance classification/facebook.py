...
class fbsession:
    ...
    def getlogin(self):
        return (
            "http://api.facebook.com/login.php?api_key="
            + apikey
            + "&auth_token="
            + self.token
        )
    ...
    def getsession(self):
        doc = self.sendrequest(
            {"method": "facebook.auth.getSession", "auth_token": self.token}
        )
    ...
    def getinfo(self, users):
        ...
        fields = (
            "gender,current_location,relationship_status,"
        ...
        for n, id in zip(doc.getElementsByTagName("result_elt"), users):
            ...
            results[id] = {
                "gender": getsinglevalue(n, "gender"),
